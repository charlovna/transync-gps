...
export default function MapPage() {
  ...
  // ── Dynamic ETA ───────────────────────────────────────────────────────────
  const originalEtaMpsRef = useRef(0);
  ...
  // ── Read dashboard route from sessionStorage on mount ─────────────────────
  useEffect(() => {
    const stored = sessionStorage.getItem("transync_dashboard_route");
    if (!stored) return;
    try {
      const parsed = JSON.parse(stored);
      sessionStorage.removeItem("transync_dashboard_route");
      ...
      if (Array.isArray(parsed.waypoints)) setWaypointInputs(parsed.waypoints);
      hasDashboardRouteRef.current = true;
    } catch { /* ignore */ }
  }, []);
  ...
  // ── Auto-trigger route when GPS ready + dashboard data present ───────────
  useEffect(() => {
    if (!hasDashboardRouteRef.current || !currentPosition || !authChecked || autoTriggeredRef.current) return;
    if (!destination.trim()) return;
    autoTriggeredRef.current = true;
    handleGetRoute();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPosition, authChecked, destination]);
  ...
  const handleEndTrip = () => {
    ...
    setShowPlanner(true); setShowAdvisory(false);
